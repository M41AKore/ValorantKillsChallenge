...
namespace ValorantKillsChallenge
{
	public class Overlay : IDisposable
	{
		...
		public Overlay()
		{
			...
			_window = new GraphicsWindow(0, 0, 250, 200, gfx)
			{
				FPS = 60,
				IsTopmost = true,
				IsVisible = true
		...
		private void _window_DrawGraphics(object sender, DrawGraphicsEventArgs e)
		{
			...
			var padding = 16;
			var infoText = new StringBuilder().Append("Overlay FPS: ").Append(gfx.FPS.ToString().PadRight(padding)).ToString();

			gfx.ClearScene(_brushes["background"]);

			gfx.DrawTextWithBackground(_fonts["consolas"], _brushes["green"], _brushes["black"], 58, 20, infoText);

			gfx.DrawTextWithBackground(_fonts["consolas"], _brushes["green"], _brushes["black"], 120, 60, "Kills: " + MainWindow.Instance.viewModel.KillCount.ToString());
